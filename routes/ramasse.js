...
destination: (_req, file, cb) => cb(null, TMP_DIR),
...
const titleY = 70;
doc.font("Helvetica-Bold").fillColor(blue).fontSize(24)
.text("Demande de ramasse de pièces", pageLeft, titleY, { align: "center", width: pageRight - pageLeft });
...
<div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;line-height:1.45;color:#111"> <p>Bonjour,</p> <p>Merci d'effectuer une <strong>ramasse</strong> chez <strong>${esc(fournisseur)}</strong> pour la/les référence(s) suivante(s) :<br/><em>${esc(pieces || "—")}</em>.</p> <p><strong>Destinataire(s) magasin :</strong> ${esc(magasinDest || "—")}<br/> <strong>Demandeur :</strong> ${esc(email)}</p> ${commentaire ? `<p><strong>Commentaire :</strong><br/>${esc(String(commentaire)).replace(/\n/g,"<br/>")}</p>` : ""} <p> <a href="${ackUrl}" style="display:inline-block;background:#2563eb;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;font-weight:700">Accuser de réception</a> </p> </div>`; }
...
if (!sup) return res.status(400).json({ error: "Fournisseur inconnu dans suppliers.json" });
