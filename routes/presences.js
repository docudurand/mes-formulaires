...
const FTP_ROOT =
  (process.env.FTP_BACKUP_FOLDER || "/").replace(/\/$/, "") + "/presences";
...
async function tryDownloadJSON(client, remotePath) {
  ...
  try {
    ...
    return null; // inexistant ou autre erreur => on traite comme vide
  } finally {
...
// 4) Résumé mensuel (TOUS les magasins – peut être plus lent)
router.get("/month", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "month_failed", message: e?.message || String(e) });
  }
});

// 4-bis) Résumé mensuel pour UN magasin (rapide & robuste)
router.get("/month-store", async (req, res) => {
  const month = String(req.query.yyyymm || "");
  const mag   = String(req.query.magasin || "");
  if (!month || !mag) return res.status(400).json({ error: "missing params" });
  console.log("[PRES] MONTH-STORE", { month, mag });

  try {
    const files = {};
    await withFtp("month-store", async (client) => {
      const json = await tryDownloadJSON(client, `${FTP_ROOT}/${month}/${mag}.json`);
      files[mag] = json || {};
    });
    ...
    try {
      if (process.env.GS_PRESENCES_URL) {
        const resp = await fetch(`${process.env.GS_PRESENCES_URL}?action=personnel&magasin=${encodeURIComponent(mag)}`);
        personnel[mag] = resp.ok ? await resp.json() : { employes: [], interims: [], livreurs: {} };
      } else {
        personnel[mag] = { employes: [], interims: [], livreurs: {} };
      }
    } catch {
      personnel[mag] = { employes: [], interims: [], livreurs: {} };
    }
    ...
    res.status(500).json({ error: "month_store_failed", message: e?.message || String(e) });
  }
});

// 5) Diag (tester connexion FTP)
router.get("/_diag", async (_req, res) => {
  try {
    const ok = await withFtp("diag", async (client) => {
      await client.cd("/"); // si accessible
      return true;
    });
    res.json({ ok });
  } catch (e) {
