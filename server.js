...
// ✅ Mailer centralisé (Gmail/SMTP) — transporter + expéditeur par défaut
...
const daysOfMonth = (ym) => {
  ...
  const out = [];
  for (let i = 1; i <= new Date(y, m, 0).getDate(); i++) out.push(new Date(y, m - 1, i));
  return out;
};
...
async function fetchMonthStoreJSON(req, ym, magasin) {
  ...
  const url = `${base}/presence/month-store?yyyymm=${encodeURIComponent(ym)}&magasin=${encodeURIComponent(
    magasin
  )}`;
  const headers = {};
...
app.get("/presence/export-month", async (req, res) => {
  try {
    ...
    for (const mag of MAGASINS_EXPORT) {
      const { file = {}, personnel = { employes: [], interims: [], livreurs: {} } } = await fetchMonthStoreJSON(
        req,
        ym,
        mag
      );
      ...
      const knownNow = new Set([...Array.from(setEmpInt.values()), ...Object.keys(personnel.livreurs || {}).map(norm)]);
      ...
      const writeSection = (title, labels, slots) => {
        ...
        labels.forEach((lbl) => {
          ...
          days.forEach((d) => {
            ...
            let values = {};
            if (rec?.data?.rows) {
              const found = rec.data.rows.find(
                (r) => String(r.label || "").trim().toUpperCase() === String(lbl).trim().toUpperCase()
              );
              values = found?.values || {};
            }
...
const PDF_DIR_PREFS = [`${FTP_ROOT_BASE}/presence/leave`, `${PRES_ROOT}/leave`];
  ...
  for (let i = 0; i < 3; i++) {
    ...
    try {
      ...
      if (!item) {
        ...
        const base = `${item.createdAt.slice(0, 10)}_${safe(item.magasin)}_${safe(item.nom)}_${safe(
          item.prenom
        )}_${item.id}.json`;
...
async function patchLeave(id, patch) {
  ...
  try {
    ...
    if (i >= 0) {
      ...
      const safe = (s) => String(s || "").normalize("NFKD").replace(/[^\w.-]+/g, "_").slice(0, 64);
      const base = `${arr[i].createdAt.slice(0, 10)}_${safe(arr[i].magasin)}_${safe(arr[i].nom)}_${safe(
        arr[i].prenom
      )}_${arr[i].id}.json`;
...
async function makeLeavePdf({ logoUrl, magasin, nomPrenom, service, nbJours, du, au, signatureData }) {
  ...
  doc.fontSize(18).font("Helvetica-Bold").text(titleStr, titleX, logoY + 45, {
    width: titleWidth,
    align: "left",
  });
...
app.post("/conges/api", async (req, res) => {
  try {
    ...
    const duFR = fmtDateFR(dateDu),
      auFR = fmtDateFR(dateAu);
...
app.post("/conges/sign/:id", async (req, res) => {
  ...
  try {
    ...
    const signerName = String(fullName || "").trim();
    ...
    // ✅ Mail final quand double signature
    if (bothSigned && !arr[i].finalMailSent) {
      try {
        if (!transporter) {
          ...
          const htmlFinal = `
            <p>Le document PDF a été <b>signé par les deux responsables</b>.</p>
            <p>
              Employé : ${(arr[i].nom || "").toUpperCase()} ${arr[i].prenom || ""}<br>
              Période : ${arr[i].dateDu} → ${arr[i].dateAu} • ${arr[i].nbJours || "?"} jour(s)
            </p>
            <p>
